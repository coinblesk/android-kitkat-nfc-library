 ...
 */
final public class PaymentMessage {
    ...
    private PaymentMessage(final Type type) {
        this.type = type;
    }
    ...
    public static PaymentMessage fromBytes(final byte[] data, PublicKey publicKey) throws InvalidKeySpecException, UnsupportedEncodingException, NoSuchProviderException, SignatureException, NoSuchAlgorithmException, InvalidKeyException {
        if(data.length<1) {
        ...
        final PaymentMessage paymentMessage = new PaymentMessage(type);
        switch (type) {
            case CONTACT_AND_PAYMENT_REQUEST:
            case CONTACT_AND_PAYMENT_RESPONSE_OK:
                //public key and user
    ...
    public boolean equals(final Object o) {
        ...
        final PaymentMessage p = (PaymentMessage) o;
    ...
    public static PaymentMessage contactAndPaymentRequest(final PublicKey publicKey, final String user, final byte[] btAddress, final long satoshis, final byte[] sendTo) {
    	if(publicKey == null) {
    		throw new RuntimeException("public key cannot be null");
    	}
    	if(user == null) {
        ...
        final PaymentMessage paymentMessage = new PaymentMessage(Type.CONTACT_AND_PAYMENT_REQUEST);
        paymentMessage.publicKey = publicKey;
        paymentMessage.user = user;
        paymentMessage.btAddress =btAddress;
    ...
    public static PaymentMessage contactAndPaymentResponseOk(final PublicKey publicKey, final String user, final byte[] btAddress, final byte[] halfSignedTransaction) {
        final PaymentMessage paymentMessage = new PaymentMessage(Type.CONTACT_AND_PAYMENT_RESPONSE_OK);
        paymentMessage.publicKey = publicKey;
        paymentMessage.user = user;
        paymentMessage.btAddress =btAddress;
    ...
    public static PaymentMessage fromServerRequestOk(final byte[] fullySignedTransaction) {
        final PaymentMessage paymentMessage = new PaymentMessage(Type.FROM_SERVER_REQUEST_OK);
        paymentMessage.fullySignedTransaction = fullySignedTransaction;
        return paymentMessage;
    }

    public static PaymentMessage fromServerRequestNok() {
        final PaymentMessage paymentMessage = new PaymentMessage(Type.FROM_SERVER_REQUEST_NOK);
        return paymentMessage;
    }

    public static PaymentMessage contactAndPaymentResponseNok() {
        final PaymentMessage paymentMessage = new PaymentMessage(Type.CONTACT_AND_PAYMENT_RESPONSE_NOK);
        return paymentMessage;
    }
